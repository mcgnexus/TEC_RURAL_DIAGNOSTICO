...
export async function findUserByTelegramId(telegramId) {
  ...
  if (error) {
    console.error('[telegramAuth] Error buscando usuario por telegram_id:', error);
    return null;
  }
...
export async function generateLinkToken(telegramId, userId = null) {
  ...
  if (error) {
    console.error('[telegramAuth] Error creando token de vinculación:', error);
...
export async function validateAndLinkToken(token, userId) {
  ...
  if (tokenError) {
    console.error('[telegramAuth] Error buscando token:', tokenError);
  ...
  if (updateError) {
    console.error('[telegramAuth] Error vinculando telegram_id:', updateError);
...
export async function createUserFromTelegram(userData) {
  ...
  if (createError) {
    console.error('[telegramAuth] Error creando usuario:', createError);
    throw new Error('Error al crear usuario: ' + createError.message);
  }

  console.log(`[telegramAuth] Usuario creado desde Telegram: ${newProfile.id} (${email})`);
...
export async function getFullProfile(telegramId) {
  ...
  if (error) {
    console.error('[telegramAuth] Error obteniendo perfil completo:', error);
    return null;
  }
