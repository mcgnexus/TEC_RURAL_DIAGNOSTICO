...
export async function downloadWhatsAppMedia(mediaUrl) {
  ...
  console.log('[whapi] Descargando media desde:', mediaUrl);
  console.log('[whapi] Usando token:', WHAPI_TOKEN ? `${WHAPI_TOKEN.substring(0, 10)}...` : 'NO CONFIGURADO');
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error('[whapi] Body del error:', errorBody);
      throw new Error(`Error descargando media: ${res.status} - ${errorBody}`);
    }
    ...
    console.error('[whapi] Error en downloadWhatsAppMedia:', error);
    throw error;
  }
}
