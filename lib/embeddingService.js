    ...
    try {
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
        ...
        if (!retryable || attempt === maxRetries) {
          ...
          console.error(`  ❌ Error final de API para lote ${batchNumber}: ${response.status} - ${response.statusText}. Cuerpo:`, errorBody);
          const errorData = await response.json().catch(() => ({}));
          throw new Error(
            `Error de Mistral API: ${response.status} - ${errorData.message || response.statusText}`
          );
      ...
      try {
        ...
        console.error('  ❌ Error parseando JSON de la respuesta. Cuerpo:', responseBody);
        throw new Error('No se pudo parsear la respuesta JSON de Mistral API');
      }
      

      if (!data?.data || data.data.length !== batch.length) {
        console.error('  ❌ Respuesta inválida de Mistral API para batch. Data:', data);
      ...
      throw new Error(`Error al generar embeddings (batch ${i / batchSize + 1}): ${error.message}`);
    }
  }
