...
async function telegramRequest(method, payload) {
  ...
  try {
    ...
    if (!res.ok || !data.ok) {
      console.error(`Error en la petición a Telegram: ${method}`, redactForLog(data));
      throw new Error(`Error en Telegram: ${data.description || res.status}`);
    }

    return data.result;
  } catch (error) {
    console.error(`Fallo al realizar la petición a Telegram (${method}):`, redactForLog(error));
    throw error;
  }
}
...
export async function sendTelegramMessage(telegramId, text, options = {}) {
  ...
  if (!text) throw new Error('El texto del mensaje no puede estar vacio.');
...
function splitTelegramText(text, maxLength = TELEGRAM_MAX_MESSAGE) {
  ...
  while (remaining.length > maxLength) {
    ...
    if (breakIndex < Math.floor(maxLength * 0.4)) {
...
export async function sendTelegramMessageChunks(telegramId, text, options = {}) {
  const chunks = splitTelegramText(text);
  for (const chunk of chunks) {
    await sendTelegramMessage(telegramId, chunk, options);
  }
}
...
export async function downloadTelegramFile(fileId) {
  ...
  console.log('[telegram] Obteniendo información del archivo:', maskId(fileId));
  ...
  console.log('[telegram] Descargando archivo desde:', redactString(downloadUrl));
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error('[telegram] Body del error:', redactString(errorBody));
    ...
    console.error('[telegram] Error en downloadTelegramFile:', redactForLog(error));
    throw error;
  }
}
