...
export async function downloadWhatsAppMedia(mediaUrl) {
  ...
  // Permitir hosts de Whapi y wasabisys (para URLs directas)
  if (!isWhapiMediaHost(host) && !host.includes('wasabisys') && !host.includes('amazonaws')) {
    console.warn('[whapi] Host de media no permitido:', host);
    console.log('[whapi] Hosts permitidos:', 'whapi.cloud, wasabisys, amazonaws');
    throw new Error(`La URL del media no es válida para descargar. Host no permitido: ${host}`);
  }
